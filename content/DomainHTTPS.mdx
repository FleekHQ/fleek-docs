...
## Terminal DNS
Terminal Sites also has the ability to handle DNS management to allow for more advanced domain and deployment features
## Custom Domains
 You can choose a custom domain to deploy your website or use a procedurally generated domain name. 
Adding a Domain [ADD PICTURES HERE]
...
## HTTPS (SSL)
Terminal offers HTTPS on all sites using the SSL protocol 
Terminal automatically manages certificate creation and renewal. Your certificate will include all your domain names when it's issued, but note that the DNS also needs to be configured in advance for each alias for us to include them on your certificate.

## Definitions
Domain name (or domain for short): full name used to access a site. For example, yoursitename.Terminal.com or www.yourcustomdomain.com
