...
Once your app is deployed on fleek, go to the menu. on the settings page in the left-hand bar menu click`Domain Management` and scrolling a bit would land you on the section that says `ENS`
...
Click Add ENS to add your ENS domain, for example `fleek.eth`. Once your domain has been added, verify you are the owner or registrant of the ENS domain

![](images/ens2.png)

## Set Fleek as the Controller

Fleek needs to be a controller of your ENS domain so we can automatically update your domain content hash when a site deployment succeeds. Click the `Set Fleek as Controller` button above to set this up:

![](images/ens3.png)

NOTE: Your domain will appear in the settings page and can also be deleted from there, you can also regain control of your domain by visiting your app.ens.domains dashboard

Finally, If the domain is using the old ENS resolver, you would be prompted to migrate to the latest resolver.

Clicking on the `Migrate` button will redirect you to the app.ens.domain dashboard to complete the migration. After successful migration, you can complete setting fleek as the domain controller.
